# ===
# 從Apple Search Ads 獲得 keywrod的searchterm 資訊，並匯出xlsx檔案
...
tryCatch({
  ...
  if (length(orgId_list) > 0) {
    # 建立初始各帳號searchterm彙總表
    searchterm_insight_output <- NULL
                  ...
                  if (apple_search_ads_request$status_code == 200){
                    ...
                    if (is.data.frame(searchterm_json_data)){
                      ...
                      if (nrow(searchterm_insight) > 0){
                        ...
                        searchterm_insight_output <- rbind(searchterm_insight_output, searchterm_insight)
                      }
                    }
    ...
    write.xlsx(searchterm_insight_output, file.path(outputpath, "searchterm_insight.xlsx"), overwrite = TRUE)
